...
(async()=>{
 for(const url of ['/','/src/beta_client.js','/beta/','/privacy-policy/'])assert.equal((await fetch(base+url)).status,200);
 for(const url of ['/.env','/server/beta_storage.js','/.git/config','/src/..%2Fserver/beta_storage.js'])assert.equal((await fetch(base+url)).status,404);
 assert.equal((await fetch(base+'/api/gemini')).status,405);
 assert.equal((await post('triage-save',{})).status,403);
 const consent=await post('consent-logs',{scope:'archive',consentVersion:'2026-09-08-beta-v2',consents:{terms:true,privacy:true,healthData:true}});assert.equal(consent.status,200);
