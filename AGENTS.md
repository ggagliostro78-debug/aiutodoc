...
## Trasferimento della beta sul branch main

- Il rilascio della beta sul branch `main` è stato autorizzato esplicitamente dall'utente. Conservare il backup indicato in `BETA-REVIEW.md` prima di ulteriori modifiche sostanziali.
- Non trasferire mai file `.env`, credenziali, service account, archivi di audit contenenti casi di prova o output generati. Le variabili BETA_* devono essere configurate solo nel pannello server-side dell'hosting.
- La pubblicazione mantiene avvisi sanitari, consensi, noindex e analytics disattivati finché non saranno completate le verifiche cliniche, privacy e cloud documentate.
- Eseguire `npm run verify`, `npm run build` e il controllo di assenza segreti in `dist` prima di ogni deploy.
