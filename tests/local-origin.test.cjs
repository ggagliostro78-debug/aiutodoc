...
test('local origin follows the configured port; production remains restricted',()=>{
 const keys=['AIUTODOC_LOCAL_MODE','AIUTODOC_PORT','AIUTODOC_ALLOWED_ORIGIN','NETLIFY','NODE_ENV'];
 const previous=Object.fromEntries(keys.map(key=>[key,process.env[key]]));
 const origin=value=>validateOrigin({headers:{origin:value}});
 try {
  process.env.AIUTODOC_LOCAL_MODE='true';delete process.env.NETLIFY;delete process.env.NODE_ENV;delete process.env.AIUTODOC_PORT;
  ...
  process.env.AIUTODOC_PORT='4384';
